...
public class AccessController : Controller
{
    ...
    public async Task<IActionResult> Register(Usuarios usuario)
    {
        ...
        usuario.TokenRecovery = Guid.NewGuid() + "-" + new Random().Next(100000, 999999);
        ...
        if (usuario.IdUsuario != 0) return RedirectToAction("Login", "Access");
    ...
    public async Task<IActionResult> Login(UsuarioVM usuario)
    {
        ...
        var claims = new List<Claim>
        {
            ...
            new(ClaimTypes.Name, usuarioRecibido.Nombre),
            new(ClaimTypes.Email, usuarioRecibido.CorreoElectronico),
            new(ClaimTypes.Role, usuarioRecibido.IdRolNavigation.Nombre)
        };
    ...
    public async Task<IActionResult> Recovery(RecoveryViewModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            var token = Guid.NewGuid() + "-" + new Random().Next(100000, 999999);
    ...
    public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
    {
        if (ModelState.IsValid)
        {
            ...
            user.TokenRecovery = Guid.NewGuid() + "-" + new Random().Next(100000, 999999);
            user.UltimaActualizacion = DateTime.Now;
    ...
    private async Task<bool> SendRecoveryEmail(string email, string resetLink)
    {
        try
        {
            ...
            using (var smtp = new SmtpClient("smtp.gmail.com"))
            {
                smtp.Port = 587;
                smtp.Credentials =
                    new NetworkCredential("no.reply.kindercare@gmail.com", "nqjf dtac vlkp shor");
                smtp.EnableSsl = true;
